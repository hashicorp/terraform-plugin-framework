...
func TestCloseEphemeralResourceRequest(t *testing.T) {
	...
	testCases := map[string]struct {
		input                   *tfprotov6.CloseEphemeralResourceRequest
	...
	for name, testCase := range testCases {
		t.Run(name, func(t *testing.T) {
			...
			got, diags := fromproto6.CloseEphemeralResourceRequest(context.Background(), testCase.input, testCase.ephemeralResource, testCase.ephemeralResourceSchema)
