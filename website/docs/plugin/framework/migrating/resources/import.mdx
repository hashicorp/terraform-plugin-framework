...
state. The following example implements resource import with SDKv2. The rest of the `schema.Resource` struct is omitted
...
The `StateContextFunc` is the function called to import a resource into Terraform state. Any operations requiredthat
are needed to import the resource take place within this function and may result in the mutation of the `ResourceData`
that is passed into the function. The return value is a slice of `schema.ResourceData`. This slice, which may be simple, likemight be as simple as returning the `ResourceData` that was passed into the function, or it may involve multipleinvolve fan out to multiple resources, such as AWS security groups.

## Framework

In the Framework, you implement the `ResourceWithImportState` interface on your  `resource.Resource` type to allow
users to import a given resource.

The following code shows how you define an `ImportState` function with the Framework. The body of the `ImportState`
function is omitted for clarity (denoted by `...`).
...
The `ImportState` function includes a `resource.ImportStateResponse`, which you use to mutate and set your resource's
state by mutating the state.
...
functions can only access the value (e.g., `ID`) supplied to the `terraform import` command.
