...
If the logic needs to return [warning or error diagnostics](/terraform/plugin/framework/diagnostics), they can added into the [`function.RunResponse.Diagnostics` field](https://pkg.go.dev/github.com/hashicorp/terraform-plugin-framework/function#RunResponse.Diagnostics).
