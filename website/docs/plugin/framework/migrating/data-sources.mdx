...
The following example shows a typical implementation. The rest of the `schema.Provider` struct is omitted for clarity
(denoted by `...`).
...
In the Framework, you define your provider's data sources adding them to your provider's `GetDataSources` function.

The `GetDataSources` function on your `provider.Provider` returns a map from the data source name (string) to a struct
...
The body of the function is omitted from this example for clarity, denoted by (`...`).
