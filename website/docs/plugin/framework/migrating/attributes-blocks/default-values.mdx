...
The following code shows a basic implementation of a default value for a primitive attribute type in SDKv2. For clarity,
the body of the attribute's schema is omitted (denoted by `...`).
...
In the Framework, you set default values with the `AttributePlanModifier` field on your attribute's `tfsdk.Attribute`
struct.
