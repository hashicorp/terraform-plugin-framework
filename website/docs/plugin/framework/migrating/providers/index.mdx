...
[`Default`](https://pkg.go.dev/github.com/hashicorp/terraform-plugin-sdk/helper/schema#Schema.Default) or
[`DefaultFunc`](https://pkg.go.dev/github.com/hashicorp/terraform-plugin-sdk/helper/schema#Schema.DefaultFunc)
you **must** remove the usage of `Default` and `DefaultFunc` and transfer the logic into the provider
[ConfigureFunc](https://pkg.go.dev/github.com/hashicorp/terraform-plugin-sdk/v2/helper/schema#Provider.ConfigureFunc) or
...
Failure to do this will like result in the following error:
...
`map[string]schema.Block`, respectively. Refer to [Providers - Schema](/plugin/framework/providers#schema) in the
