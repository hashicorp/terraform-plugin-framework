...
func TestGenerateResourceConfigRequest(t *testing.T) {
	...
	testResource := &testprovider.Resource{}

	testCases := map[string]struct {
		input               *tfprotov5.GenerateResourceConfigRequest
		resource            resource.Resource
		resourceSchema      fwschema.Schema
		expected            *fwserver.GenerateResourceConfigRequest
	...
	for name, testCase := range testCases {
		t.Run(name, func(t *testing.T) {
			...
			got, diags := fromproto5.GenerateResourceConfigRequest(t.Context(), testCase.input, testCase.resource, testCase.resourceSchema)
