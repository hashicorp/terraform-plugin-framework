...
type that implements the `provider.Provider` interface, which you must define.
...
satisfying the `provider.Provider` interface. Refer to [Providers](/plugin/framework/providers) in the Framework
documentation for details.

The following ocde shows a typical implementation. In this implementation, the `GetResources` function returns a map
