...
Refer to [Attributes](/plugin/framework/handling-data/attributes) for further details of the different types of attributes.
...
During execution of the [`terraform validate`](/cli/commands/validate), [`terraform plan`](/cli/commands/plan) and [`terraform apply`](/cli/commands/apply) commands, Terraform calls the provider [`ValidateProviderConfig`](/plugin/framework/internals/rpcs#validateproviderconfig-rpc), [`ValidateResourceConfig`](/plugin/framework/internals/rpcs#validateresourceconfig-rpc) and [`ValidateDataResourceConfig`](/plugin/framework/internals/rpcs#validatedataresourceconfig-rpc) RPCs, during which [value validation](/plugin/framework/validation) takes place.
