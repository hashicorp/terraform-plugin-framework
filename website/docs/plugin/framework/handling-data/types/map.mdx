...
In this example, a map of strings value is checked for being null or unknown value first, before accessing its known value elements as a `[]types.String`:
...
listValue, diags := types.MapValue(types.StringType, elements)
```
...
listValue, diags := types.MapValueFrom(ctx, types.StringType, elements)
```

## Extending

The framework supports extending its base type implementations with [custom types](/terraform/plugin/framework/handling-data/custom-types). These can adjust expected provider code usage depending on their implementation.
