...
Set function parameters expect an unordered, unique collection of single element type value from a practitioner configuration. Values are accessible in function logic by a Go slice of an appropriate type to match the element type `[]T` or the [framework set type](/terraform/plugin/framework/handling-data/types/set).
...
* Otherwise, use the Go slice of an appropriate type to match the element type `[]T` or [framework set type](/terraform/plugin/framework/handling-data/types/set).
