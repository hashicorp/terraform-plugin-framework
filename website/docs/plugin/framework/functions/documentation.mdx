...
| Field Name | Description |
|---|---|
| `Name` | Single word or abbreviation of parameter for function signature generation. If name is not provided, will default to the prefix "param" with a suffix of the position the parameter is in the function definition. (`param1`, `param2`, etc.). If the parameter is variadic, the default name will be `varparam`. |
...
The name must be unique in the context of the [function definition](/terraform/plugin/framework/functions/implementation#definition-method) and is only for documentation purposes. The name can be helpful when there is a need to disambiguate between multiple parameters, such as the words `cidr` and `ip` in a generated function signature like `cidr_contains_ip(cidr string, ip string) bool`.
