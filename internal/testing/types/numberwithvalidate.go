...
func (t NumberTypeWithValidateError) Validate(ctx context.Context, in tftypes.Value, path attrpath.Path) diag.Diagnostics {
	return diag.Diagnostics{TestErrorDiagnostic(path)}
}

func (t NumberTypeWithValidateWarning) Validate(ctx context.Context, in tftypes.Value, path attrpath.Path) diag.Diagnostics {
