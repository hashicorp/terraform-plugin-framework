...
func (f *EchoFunction) Run(ctx context.Context, req function.RunRequest, resp *function.RunResponse) {
    ...
    resp.Errors.Append(req.Arguments.Get(ctx, &input)...)

    // Set the result to the same data
    resp.Errors.Append(resp.Result.Set(ctx, &input)...)
}
```
...
If the logic needs to return [errors](/terraform/plugin/framework/functions/errors), they can be added into the [`function.RunResponse.Errors` field](https://pkg.go.dev/github.com/hashicorp/terraform-plugin-framework/function#RunResponse.Errors).
...
func (f *ExampleFunction) Run(ctx context.Context, req function.RunRequest, resp *function.RunResponse) {
    ...
    resp.Errors.Append(req.Arguments.GetArgument(ctx, 0, &boolArg)...)
...
func (f *ExampleFunction) Run(ctx context.Context, req function.RunRequest, resp *function.RunResponse) {
    ...
    resp.Errors.Append(req.Arguments.Get(ctx, &boolArg, &stringVarg)...)
...
func (f *ExampleFunction) Run(ctx context.Context, req function.RunRequest, resp *function.RunResponse) {
    ...
    resp.Errors.Append(req.Arguments.Get(ctx, &boolArg, &int64arg, &stringVarg)...)

    for index, element := range stringVarg {
        // Added by 2 to match the definition including two parameters.
        resp.Errors.AddArgumentWarning(2+index, "example summary", "example detail")
    }
