...
func reflectSlice(ctx context.Context, typ attr.Type, val tftypes.Value, target reflect.Value, opts Options, path *tftypes.AttributePath) (reflect.Value, diag.Diagnostics) {
	...
	if target.Kind() != reflect.Slice {
		diags.Append(NewDiagIntoIncompatibleType(
			...
			val,
			target.Type(),
			fmt.Errorf("expected a slice type, got %s", target.Type()),
		))
		return target, diags
	}
	...
	if !ok {
		diags.Append(NewDiagIntoIncompatibleType(
			...
			val,
			target.Type(),
			fmt.Errorf("cannot reflect %s using type information provided by %T, %T must be an attr.TypeWithElementType", val.Type(), typ, typ),
		))
		return target, diags
	}
