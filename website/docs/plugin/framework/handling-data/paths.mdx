...
| `schema.DynamicAttribute` | Based on underlying value, see [Building Dynamic Attribute Paths](#building-dynamic-attribute-paths)   |
...
It's recommended when building a path for a [dynamic attribute](/terraform/plugin/framework/handling-data/attributes/dynamic) that you always target the root dynamic attribute.
...
#### Child Paths in Dynamic Attribute

Since a dynamic attribute can store any value, that means it's possible to build a path that references a child in a dynamic value (with the child path methods defined [here](#building-attribute-paths)).
...
resource "examplecloud_thing" "example" {
  root_dynamic_attribute = tolist(["one", "two"])
}
```

The path which matches the second string value (`"two"`) associated with the dynamic list value of the `root_dynamic_attribute` attribute is:

```go
path.Root("root_dynamic_attribute").AtListIndex(1)
```

If you attempt to use the same path, using `AtListIndex()`, with the following config, you'll receive an error diagnostic:
```hcl
resource "examplecloud_thing" "example" {
  root_dynamic_attribute = {
    attr1 = "not a list!"
  }
}
```

Since the correct child method depends on the underlying value, which in a dynamic attribute can be **any** value type, it's recommended to build paths matching the root dynamic attribute:
```go
path.Root("root_dynamic_attribute")
```
