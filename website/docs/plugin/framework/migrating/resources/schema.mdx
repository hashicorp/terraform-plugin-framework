...
Refer to the [Resources - CRUD functions](FIXME: link) page in this guide to learn how to define the `resource.Resource`
type returned by your resource type's `NewResource` function.
...
the `ResourceType` interface, which includes `GetSchema` that returns your resource's schema.
