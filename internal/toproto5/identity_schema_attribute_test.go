...
func TestIdentitySchemaAttribute(t *testing.T) {
	...
	tests := map[string]testCase{
		...
		"nested-attr-single-error": {
			...
			path:        tftypes.NewAttributePath(),
			expectedErr: "identity schemas and protocol version 5 don't support NestedAttribute",
		},
	...
	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			...
			got, err := toproto5.IdentitySchemaAttribute(context.Background(), tc.name, tc.path, tc.attr)
			if err != nil {
				if tc.expectedErr == "" {
