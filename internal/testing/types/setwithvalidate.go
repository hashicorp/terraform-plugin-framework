...
func (t SetTypeWithValidateError) Validate(ctx context.Context, in tftypes.Value, path attrpath.Path) diag.Diagnostics {
	return diag.Diagnostics{TestErrorDiagnostic(path)}
}

func (t SetTypeWithValidateWarning) Validate(ctx context.Context, in tftypes.Value, path attrpath.Path) diag.Diagnostics {
