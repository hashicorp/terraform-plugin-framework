...
if [[ ! -f CHANGELOG.md ]]; then
    ...
    echo "Please run this from the root of the framework repository"
    exit 1
fi
...
$SED 's/GH-([0-9]+)/\[#\1\]\(https:\/\/github.com\/hashicorp\/terraform-plugin-framework\/issues\/\1\)/g' -e 's/\[\[#(.+)([0-9])\)]$/(\[#\1\2))/g' CHANGELOG.md
