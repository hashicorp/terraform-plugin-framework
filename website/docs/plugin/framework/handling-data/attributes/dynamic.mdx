...
In this example, the same dynamic attribute is set to the ordered list collection values `one` and `two`:
...
In this example, the same dynamic attribute is set to the mapped values of `attr1` to `"value1"` and `attr2` to `123`:
...
Dynamic values are not currently supported as the element type of a [collection attribute type](/terraform/plugin/framework/handling-data/attributes#collection-attribute-types).
...
- [`StaticDynamic(types.Dynamic)`](https://pkg.go.dev/github.com/hashicorp/terraform-plugin-framework/resource/schema/dynamicdefault#StaticDynamic): Define a static dynamic default value for the attribute.
