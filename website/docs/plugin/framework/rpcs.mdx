...
The [sequence of RPCs](/plugin/terraform-plugin-protocol#rpcs-and-terraform-commands) that are issued are determined by the Terraform command that is executed.

The RPCs described are for a provider that is using version 6 of the Terraform plugin protocol. Refer to [Protocol RPCs](/plugin/terraform-plugin-protocol#protocol-rpcs) for the equivalent RPCS for protocol version 5. The same provider methods are called irrespective of the protocol version that is being used.
...
When _terraform validate | plan | apply_ are executed Terraform core issues the GetProviderSchema RPC. The RPC flows through the Terraform plugin framework and ultimately calls the Schema function on the provider and each of the resources and data sources that the provider defines.
