...
func Struct(ctx context.Context, typ attr.Type, object tftypes.Value, target reflect.Value, opts Options, path *tftypes.AttributePath) (reflect.Value, diag.Diagnostics) {
	...
	if target.Kind() != reflect.Struct {
		diags.Append(NewDiagIntoIncompatibleType(
			...
			object,
			target.Type(),
			fmt.Errorf("expected a struct type, got %s", target.Type()),
		))
		return target, diags
	}
	if !object.Type().Is(tftypes.Object{}) {
		diags.Append(NewDiagIntoIncompatibleType(
			...
			object,
			target.Type(),
			fmt.Errorf("cannot reflect %s into a struct, must be an object", object.Type().String()),
		))
		return target, diags
	}
	...
	if !ok {
		diags.Append(NewDiagIntoIncompatibleType(
			...
			object,
			target.Type(),
			fmt.Errorf("cannot reflect object using type information provided by %T, %T must be an attr.TypeWithAttributeTypes", typ, typ),
		))
		return target, diags
	}
	...
	if err != nil {
		diags.Append(NewDiagIntoIncompatibleType(
			...
			object,
			target.Type(),
			fmt.Errorf("error retrieving field names from struct tags: %w", err),
		))
		return target, diags
	}
	...
	if len(objectMissing) > 0 || len(targetMissing) > 0 {
		...
		diags.Append(NewDiagIntoIncompatibleType(
			...
			object,
			target.Type(),
			fmt.Errorf("mismatch between struct and object: %s", strings.Join(missing, " ")),
		))
		return target, diags
	}
	...
	for field, structFieldPos := range targetFields {
		...
		if !ok {
			diags.Append(NewDiagIntoIncompatibleType(
				...
				object,
				target.Type(),
				fmt.Errorf("could not find type information for attribute in supplied attr.Type %T", typ),
			))
			return target, diags
		}
