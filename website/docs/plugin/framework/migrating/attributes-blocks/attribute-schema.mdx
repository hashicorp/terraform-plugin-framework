...
The following code shows a basic implementation of attribute schema for a provider in SDKv2. For clarity, the body of
the attribute's schema is omitted (denoted by `...`).
...
The following code shows how to define an attribute for a resource with the Framework. For clarity, the body of the
`tfsdk.Attribute` struct is omitted (denoted by `...`).
