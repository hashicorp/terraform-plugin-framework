...
of the http provider. Some code is omitted for clarity (denoted by `...`).
...
For clarity, some parts of the migration are omitted (denoted by `...`). To review a complete example, clone the
...
[Provider Definition](/plugin/framework/migrating/provider#definition-framework) in the Framework documentation for
details.
