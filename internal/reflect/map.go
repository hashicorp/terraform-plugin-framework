...
func Map(ctx context.Context, typ attr.Type, val tftypes.Value, target reflect.Value, opts Options, path *tftypes.AttributePath) (reflect.Value, diag.Diagnostics) {
	...
	if underlyingValue.Kind() != reflect.Map {
		diags.Append(NewDiagIntoIncompatibleType(
			...
			val,
			target.Type(),
			fmt.Errorf("expected a map type, got %s", target.Type()),
		))
		return target, diags
	}
	if !val.Type().Is(tftypes.Map{}) {
		diags.Append(NewDiagIntoIncompatibleType(
			...
			val,
			target.Type(),
			fmt.Errorf("cannot reflect %s into a map, must be a map", val.Type().String()),
		))
		return target, diags
	}
	...
	if !ok {
		diags.Append(NewDiagIntoIncompatibleType(
			...
			val,
			target.Type(),
			fmt.Errorf("cannot reflect map using type information provided by %T, %T must be an attr.TypeWithElementType", typ, typ),
		))
		return target, diags
	}
