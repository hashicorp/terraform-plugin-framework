...
When you update the resource implementation in your provider, some changes may not be compatible with old versions. You
...
The following code shows a basic implementation of the `stateUpgraders` field in SDKv2. For clarity, the rest of the
`schema.Resource` struct is omitted (denoted by `...`).
...
In the Framework, you implement the `ResourceWithUpgradeState` on your `resource.Resource` type to upgrade your
resource's state when required.

The following code shows how you define an `UpgradeState` function with the Framework. The body of the `UpgradeState`
